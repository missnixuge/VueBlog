<template>
    <div class="home">
        ...
        <article class="newhome">
            <div class="explain">
                        <div class="title">
                          <el-divider content-position="center"><h1 class="message-divider">友链申请</h1></el-divider>
                        </div>
                        <div class="explain-field-box">
                            ...
                            <p></p>
                            <h1 class="text-center" style="text-align: center;">The End</h1>
                        </div>
            </div>
                            ...
                            <el-col v-for="(item,index) in data" :key="index" :md="6" :sm="12" :xs="24">
                                <a target="_blank" :href="item.siteurl">
                                    <div class="link-box">
                                        <el-row>
                                            <el-col :span="24">
                                                <img alt="" :src="item.siteicon">
                                            </el-col>
                                        </el-row>
                                        <div class="text-center">
                                            <h3>{{item.sitename}}</h3>
                                            <p>{{item.siteremarks}}</p>
                                        </div>
                                    </div>
                                </a>
                            </el-col>
...
<script>
 export default{
    ...
    methods: {
        init() {
            this.$api.get("FriendLink?page=1 && key = '' && intPageSize = 50"+ 0, null, r => {
                    let result = r.response.data;
                    console.log(r.response);
                    if(result.length > 0) {
                        this.data = result;//this.commentList.concat(result)
